...
func (t *Transport) buildPeerConnections(serverICE []*livekit.ICEServer) error {
	...
	// Publish the PC/track handles before wiring OnNegotiationNeeded below:
	// pion may invoke onPublisherNegotiationNeeded from its own goroutine as
	// soon as the handler is registered, and that callback reads t.publisher.
	// Assigning here (rather than in NewTransport after this returns) closes
	// the init race.
