...
| `SIP_AUTO_RINGING` | `false` | When `true`, send `180 Ringing` automatically right after `100 Trying` on every inbound INVITE (legacy behavior). When `false` (default), only `100 Trying` is sent and the API caller drives ringing explicitly via `POST /v1/legs/{id}/ring`, `/early-media`, or `/answer`. |
...
| `RTT_ENABLED` | `false` | Negotiate Real-Time Text (T.140 / RFC 4103) on SIP legs. When enabled, outbound INVITEs offer an `m=text` section and inbound INVITEs that carry one are answered. Peers without RTT support fall back to audio-only cleanly. |
| `RTT_REDUNDANCY_LEVEL` | `2` | Number of RFC 2198 RED redundancy generations applied to outgoing T.140 packets. `0` disables RED (plain T.140). |
| `RTT_BUFFER_MS` | `300` | T.140 transmission interval in ms (RFC 4103 §5.2 recommends 300 ms; max 500 ms). |
